...
export const MyHoldings = () => {
  ...
  const { queryAllNFTBlindBox ,queryAllNFTBlindBoxBySeller} = GraphqlServer();
  ...
  useEffect(() => {
    const updateMyCollectibles = async (): Promise<void> => {
      ...
      // 请求当前页的数据
      const query = await queryAllNFTBlindBox( pageSize, offset) as Query;
  ...
  return (
    <>
      {myAllNFTBlindBox.length === 0 ? (
        <div className="flex justify-center items-center mt-10">
          <div className="text-2xl text-primary-content">No NFTBlindBox found</div>
        </div>
      ) : (
        <div>
          {/* DaisyUI Pagination */}
